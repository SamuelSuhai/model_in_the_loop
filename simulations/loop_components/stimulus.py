...
def create_rf_test_tensor(heigth: int = 18, 
    ...
    for i,intensity in enumerate([255,0]):
...
def extract_selected_meis(rois_seed: List[Tuple[int,int]],
                          ...
                          neuron_id_to_roi_id: Dict[int,int]) -> Dict[str,torch.Tensor]:
    """
    ...
    for roi_id, seed in rois_seed:
        if roi_id not in neuron_id_to_roi_id:
            raise ValueError(f"roi_id {roi_id} not found in neuron_id_to_roi_id mapping.")
        neuron_id = neuron_id_to_roi_id[roi_id]
        if neuron_id not in neuron_seed_mei_dict or seed not in neuron_seed_mei_dict[neuron_id]:
...
def create_rf_mei_avi_from_roi_id_and_seed(rois_seed: List[Tuple[int,int]],
    ...
    selected_meis = extract_selected_meis(rois_seed, neuron_seed_mei_dict,neuron_id_to_roi_id)
