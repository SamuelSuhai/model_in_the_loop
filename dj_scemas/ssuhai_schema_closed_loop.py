...
class PeakSTAPositionTemplate(dj.Computed):
    ...
    def definition(self):
        definition = """
        # Peak position of the STA receptive field relatieve to the stimulus center.
        -> self.sta_table
        ---
        rf_peak_x_um: float
        rf_peak_y_um: float
    ...
    def get_peak_rel_center(self,max_indices) -> Tuple[float, float]:
        """
        """
        ...
        x_pix = max_x_idx - (pix_n_x // 2) 
        y_pix = max_y_idx - (pix_n_y // 2)

        # Calculate position in microns
        x_um = x_pix * pixel_size_x_um 
        y_um = y_pix * pixel_size_y_um         
        return (x_um, y_um)
    ...
    def make(self, key):
        ...
        peak_x_pix, peak_y_pix = self.get_rf_spatial_max_indices(rf_array)
        rf_peak_x_um, rf_peak_y_um = self.get_peak_rel_center((peak_x_pix, peak_y_pix))

        self.insert1({
            **key,
            'rf_peak_x_um': rf_peak_x_um,
            'rf_peak_y_um': rf_peak_y_um
        })
...
class OpenRetinaHoeflingFormatTemplate(dj.Manual):
    ...
    @property
    @abstractmethod
    def peak_sta_position_table(self):
        pass
    ...
    def get_query(self, key):
        ...
        # peak position of the STA receptive field: for later stimulus placement 
        peak_sta_position_table = self.peak_sta_position_table() # & key
        ...
        result = (
            ...
            celltype_table * \
            qi_table * \
            self.roi_table().proj("roi_size_um2") * peak_sta_position_table.proj(
                rf_peak_x_um="rf_peak_x_um", rf_peak_y_um="rf_peak_y_um")
        ) 
...
class OpenRetinaHoeflingFormat(OpenRetinaHoeflingFormatTemplate):
    ...
    peak_sta_position_table = PeakSTAPosition
